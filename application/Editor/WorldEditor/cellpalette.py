...
class CellPalette:
    ...
    def __init__(self, editor):
        ...
        self.set_cells(
            editor.editor_world.cell_palette
        )
    ...
    def set_cells(self, cells):
        ...
        for index, cell_id in enumerate(self.cells):
            ...
            x = (
                self.padding
                + column * (self.cell_size + self.padding)
            )
            ...
            self.rects[cell_id] = self.system.window.Rect(
                x,
                y,
                self.cell_size,
                self.cell_size
    ...
    def handle_event(self, event):
        ...
        if (
            event.type
            != self.system.input.mouse_button_down()
        ):
            return
        ...
        if not self.editor.cell_palette_rect.collidepoint(
            mx,
            my
        ):
            return
    ...
    def draw(self):
        ...
        for cell_id, rect in self.rects.items():
            ...
            surface = self.editor.coordinate_font.font.render(
                str(cell_id),
                True,
                black
            )
