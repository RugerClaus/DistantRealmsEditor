...
class WorldEditor:
    ...
    def __init__(self, distant_realms):
        ...
        self.map_selector = MapSelector(
            self.distant_realms.application,
            self
        )
    ...
    def handle_event(self, event, command):
        ...
        if self.state.is_state(WORLD_EDITOR_STATE.MAP):
            ...
            self.map_selector.handle_event(
                event,
                command
            )
        ...
        if self.map_creator.visible:
            ...
            if event.type == self.system.input.keydown():
                ...
                if event.key == self.system.input.keys.escape_key():
                    self.distant_realms.ui_controller.show_ui(
                        "world_editor_controls"
                    )
    ...
    def render_selected_world_position(self):
        ...
        for map in maps:
            ...
            # Only render the grid for the active layer.
    ...
    def render_hovered_cell(self):
        ...
        # Get the exact canvas rectangle occupied by the active map.
        map_x, map_y = self.world_to_canvas(
            map.world_x,
            map.world_y
        )

        map_width, map_height = self.world_size_to_canvas(
            map.map_width,
            map.map_height
        )
        ...
        # Mouse isn't over the active map.
        if (
            mouse_x < map_x
            or mouse_x >= map_x + map_width
            or mouse_y < map_y
            or mouse_y >= map_y + map_height
        ):
            return

        # Determine which CELL of the map the mouse is over.
        cell_width = map_width / map.columns
        cell_height = map_height / map.rows

        column = int(
            (mouse_x - map_x) / cell_width
        )

        row = int(
            (mouse_y - map_y) / cell_height
        )
        ...
        # Convert the cell coordinate back into canvas coordinates.
        cell_x = round(
            map_x + column * cell_width
        )

        cell_y = round(
            map_y + row * cell_height
        )
        ...
        overlay = self.distant_realms.system.window.make_surface(
            cell_width,
            cell_height,
            True
        )
    ...
    def get_world_position(self, mouse_pos):
        ...
        return (
            ...
            + local_x / width * self.editor_world.view_width,

            self.editor_world.view_y
            + local_y / height * self.editor_world.view_height
        )
    ...
    def scale(self):
        ...
        self.cell_palette = self.distant_realms.system.window.make_surface(
            width,
            height
        )

        self.cell_palette_rect = self.cell_palette.get_rect(
            topleft=(x, y)
        )
    ...
    def world_to_canvas(self, world_x, world_y):
        ...
        x = (
            (world_x - self.canvas_view_x)
            / self.canvas_view_width
            * self.canvas.get_width()
        )

        y = (
            (world_y - self.canvas_view_y)
    ...
    def render_cell_palette(self):
        ...
        for cell_id, cell in self.editor_world.cell_palette.items():
            ...
            rect = self.distant_realms.system.window.Rect(
                x,
                y,
                cell_size,
                cell_size
            ...
            self.distant_realms.system.window.draw_rect(
                self.cell_palette,
                color,
                rect
            )
