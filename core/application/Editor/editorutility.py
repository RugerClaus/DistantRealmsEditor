...
class EditorUtility:
    ...
    def create_project_file(self, data):
        ...
        log_event(
            ...
            f"of type: {project_type}",
            "EditorUtility.create_project_file"
        )
        ...
        if self.distant_realms.application:
            ...
            if project_type == "Menu":
                ...
                application.state.set_state(
                    EDITOR_STATE.CELLMAP
                )

                self.distant_realms.ui_controller.clear()

                self.distant_realms.ui_controller.show_ui(
                    "cell_map_editor_base"
                )

            application.editor.active_file = (
    ...
    def load_project_file(self, filename):
        ...
        self.initialize_editor_for_type(
            project_type
        )
    ...
    def load_project(
        ...
        if project_type == "menu":
            ...
            path = persistence.get_menu(
                filename
            )

        elif project_type == "form":

            path = persistence.get_form(
                filename
            )
    ...
    def display_volume(self, ui):
        ...
        if (
            ...
            .ui_controller
            .active_name
            != "editor_audio_settings"
        ):
            return

        music_vol = float(
            self.distant_realms.system.sound.volume
        )
        ...
        sfx_vol = float(
            self.distant_realms.system.sound.sfx_volume
        )
